        ...
        return ok(
            {
                "task": None,
                "mode": "red",
                "message": "Today might be a rest day. That's okay.",
            }
        )
    ...
    return ok(
        {
            "task": task,
            "mode": mode,
            "message": None if task else "No tasks fit your current energy.",
        }
    )
...
async def create_task(task: TaskCreate, store: DataStore = Depends(get_store)):
    ...
    if task.spoon_cost is None:
        resp = await call_llm(
            "Estimate spoon cost (1-5). 1=easy, 5=exhausting. Reply with JUST a number.",
            f"Task: {task.title}. {task.description}",
            max_tokens=10,
        )
        try:
            spoon_cost = max(0.5, min(5.0, float(resp.strip())))
    ...
    chunk_response = re.sub(
        r"<think>.*?</think>", "", chunk_response, flags=re.DOTALL
    ).strip()
    micro_chunks = []
    try:
        ...
        if cleaned.startswith("["):
            ...
            micro_chunks = [
                line.strip("- ").strip() for line in cleaned.split("\n") if line.strip()
            ]
