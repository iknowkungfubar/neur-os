...
async def submit_passive_log(
    data: PassiveLogSubmit, store: DataStore = Depends(get_store)
):
    lid = store.submit_passive_log(
        data.response, data.spoons_at_time, data.current_task_id, data.source
    )
...
async def check_passive_prompt(store: DataStore = Depends(get_store)):
    ...
    return ok(
        {"should_prompt": mins_ago >= 60, "last_response_minutes_ago": round(mins_ago)}
    )
