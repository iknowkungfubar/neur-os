...
async def export_markdown(store: DataStore = Depends(get_store)):
    ...
    for t in data.get("tasks", []):
        ...
        lines.append(
            f"- {icon} **{t['title']}**{rec} — {t['spoon_cost']} spoons [{t['energy_tag']}]"
        )
        lines.append("")
    lines.append("## Daily Energy\n")
    for s in data.get("daily_state", []):
        lines.append(
            f"- **{s['date']}** ({s['mode']}): {s['remaining_spoons']}/{s['total_spoons']} spoons, pain {s['pain_level']}/4"
        )
        if s.get("notes"):
    ...
    for w in data.get("wind_down", []):
        ...
        if w.get("went_well"):
            lines.append(f"- Went well: {w['went_well']}")
        if w.get("drained"):
            lines.append(f"- Drained: {w['drained']}")
        if w.get("tomorrow_one"):
            lines.append(f"- Tomorrow: {w['tomorrow_one']}")
        lines.append("")
