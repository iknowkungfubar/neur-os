...
async def brain_dump(data: BrainDumpRequest, store: DataStore = Depends(get_store)):
    ...
    if data.declarative:
        try:
            dec = await call_llm(
                (
                    "Rewrite this to be gentle, declarative, and demand-free. "
                    "Remove urgency, guilt, imperative mood. Keep the meaning."
                ),
                data.text,
                max_tokens=200,
            )
            dec = re.sub(r"questions.*?answers?", "", dec, flags=re.DOTALL).strip()
    ...
    try:
        resp = await call_llm(
            (
                "Organize this brain dump into tasks and notes. Return JSON: "
                '{"tasks": [{"title": str, "spoon_cost": 0.5-5.0, '
                '"energy_tag": "low"|"medium"|"high"}], "notes": [{"content": str}]}'
            ),
            data.text,
            max_tokens=500,
        )
        cleaned = re.sub(r"questions.*?answers?", "", resp, flags=re.DOTALL).strip()
    ...
    if not structured.get("tasks") and not structured.get("notes"):
        structured = {
            "tasks": [{"title": text, "spoon_cost": 1.0, "energy_tag": "medium"}],
            "notes": [],
        }
    if structured.get("tasks"):
        for t in structured["tasks"]:
            store.create_task(
                {
                    "title": t.get("title", text),
                    "description": "",
                    "spoon_cost": t.get("spoon_cost", 1.0),
                    "micro_chunks": [],
                    "energy_tag": t.get("energy_tag", "medium"),
                    "recurring": "",
                }
            )
