...
class InMemoryStore(DataStore):
    ...
    def log_energy(self, spoons: float, pain: int = 0, note: str = "") -> str:
        lid = uuid.uuid4().hex
        self.energy_logs.insert(
            0,
            {
                "id": lid,
                "timestamp": self._ts(),
                "spoons_remaining": spoons,
                "pain_level": pain,
                "note": note,
            },
        )
    ...
    def energy_patterns(self) -> dict:
        ...
        return {
            "by_hour": [
                {"hour": h, "avg_energy": round(hour_avg[h], 1)}
                for h in sorted(hour_avg)
            ],
            "by_day": [
                {"day": d, "avg_energy": round(dow_avg[d], 1)}
                for d in days
                if d in dow_avg
            ],
    ...
    def get_tasks(self, status_filter: str | None = None) -> list[dict]:
        all_t = sorted(
            self.tasks.values(),
            key=lambda t: (t.get("energy_tag", "medium"), t.get("created_at", "")),
        )
        if status_filter:
    ...
    def next_task(self, mode: str, remaining: float) -> dict | None:
        ...
        if mode == "amber":
            affordable = [
                t
                for t in active
                if t["spoon_cost"] <= min(remaining, 2)
                and t["energy_tag"] in ("low", "medium")
            ]
            return (
                sorted(affordable, key=lambda t: t["spoon_cost"])[0]
                if affordable
                else None
            )
    ...
    def get_habits(self, today: str) -> list[dict]:
        return [
            {**h, "done_today": h.get("last_completed") == today}
    ...
    def create_habit(self, data: dict) -> str:
        ...
        self.habits[hid] = {
            ...
            "spoon_cost": data.get("spoon_cost", 0.5),
            "energy_tag": data.get("energy_tag", "low"),
            "last_completed": None,
            "created_at": self._ts(),
        }
        return hid
    ...
    def get_active_timer(self) -> dict | None:
        active = [
            t for t in self.timers.values() if t["status"] in ("running", "paused")
        ]
        if not active:
            return None
    ...
    def search_brain_dumps(self, q: str) -> list[dict]:
        return [
            b
            for b in self.brain_dumps
            if q.lower() in b["raw_text"].lower()
            or q.lower() in b["structured_json"].lower()
        ][:10]

    def list_brain_dumps(self, limit: int = 20) -> list[dict]:
        return sorted(
            self.brain_dumps, key=lambda b: b.get("created_at", ""), reverse=True
        )[:limit]
    ...
    def get_dopamine_menu(self) -> dict[str, list[dict]]:
        ...
        for item in sorted(
            self.dopamine_menu_items, key=lambda i: i.get("sort_order", 99)
        ):
            d = dict(item)
            cat = d.pop("category")
            if cat in menu:
    ...
    def delete_dopamine_item(self, item_id: str) -> None:
        self.dopamine_menu_items = [
            i for i in self.dopamine_menu_items if i["id"] != item_id
        ]
    ...
    def last_passive_log_today(self) -> dict | None:
        today = date.today().isoformat()
        today_logs = [
            p for p in self.passive_logs if p.get("timestamp", "")[:10] == today
        ]
        return (
            sorted(today_logs, key=lambda p: p.get("timestamp", ""), reverse=True)[0]
            if today_logs
            else None
        )
    ...
    def save_onboarding(self, phase: int, turns: int, profile: dict) -> None:
        self.onboarding = {
            "phase": phase,
            "turns": turns,
            "extracted_profile": json.dumps(profile),
            "updated_at": self._ts(),
        }
                ...
                "items": [
                    {
                        "name": i["name"],
                        "category": i["category"],
                        "energy_required": i.get("energy_required", 0.5),
                    }
                    for i in self.dopamine_menu_items
                ],
            }
            ...
            self.dopamine_menu_items.append(
                {
                    ...
                    "energy_required": item.get("energy_required", 0.5),
                    "sort_order": 99,
                }
            )
            count += 1
    ...
    def sync_download(
        ...
        matches = [
            s
            for s in self.sync_data
            if s["device_id"] == device_id and s["collection"] == collection
        ]
        if since:
            matches = [s for s in matches if s.get("created_at", "") > since]
        return sorted(matches, key=lambda s: s.get("created_at", ""), reverse=True)[
            :100
        ]
    ...
    def weekly_review(self, week_ago: str, today: str) -> dict:
        ...
        completed = [
            ...
            if t.get("completed_at") and t["completed_at"][:10] >= week_ago
        ]
        crises = [c for c in self.crises if c.get("timestamp", "")[:10] >= week_ago]
        habits = list(self.habits.values())
        wind = [wd for d, wd in self.wind_downs.items() if d >= week_ago]
        timers = [
            t for t in self.timers.values() if t.get("started_at", "")[:10] >= week_ago
        ]

        avg_spoons = (
            sum(float(s["total_spoons"]) for s in energy_states) / len(energy_states)
            if energy_states
            else 0
        )
        avg_pain = (
            sum(s["pain_level"] for s in energy_states) / len(energy_states)
            if energy_states
            else 0
        )
        focus_min = sum(
        ...
        return {
            ...
            "energy_entries": self.energy_logs,
            "timer_sessions": timers,
            "crises": crises,
            "habits": habits,
            "wind_down_entries": wind,
            "insights": {
    ...
    def review_insight(self, week_ago: str) -> dict:
        ...
        if days < 2:
            ...
            lines.append(
                f"Completed **{total_tasks} tasks** ({high_spoon} high-energy)."
                if total_tasks > 0
                else "No tasks completed this week - that's okay."
            )
            if total_crises:
                lines.append(
            ...
            if energy_states:
                ...
                lines.append(
                    f"Highest energy day: **{best['date']}** ({best['remaining_spoons']}/{best['total_spoons']} spoons)."
                )
                worst = min(energy_states, key=lambda s: s["remaining_spoons"])
                if worst["remaining_spoons"] < best["remaining_spoons"]:
                    lines.append(
                        f"Lowest energy day: **{worst['date']}** ({worst['remaining_spoons']}/{worst['total_spoons']} spoons)."
                    )
            if wind:
                themes = [
                    e["went_well"].split(".")[0] for e in wind if e.get("went_well")
                ]
                if themes:
